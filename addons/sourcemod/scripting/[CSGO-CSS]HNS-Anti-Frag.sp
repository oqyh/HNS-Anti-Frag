...
{
	name = "[CSGO-CSS] HNS Anti Frag",
	author = "Gold KingZ ",
	description = "Modify Knife Damage + Cooldown From Stabbing T's and More",
...
{
	LoadTranslations( "[CSGO-CSS]HNS-Anti-Frag.phrases" );
	
	CreateConVar("hns_f_version", PLUGIN_VERSION, "[CSGO-CSS] HNS Anti Frag Plugin Version", FCVAR_NOTIFY|FCVAR_DONTRECORD);

	h_enable_plugin = CreateConVar("hns_f_enable_plugin", "1", "Enable [CSGO-CSS] HNS Anti Frag Plugin?\n1= Yes\n0= No", _, true, 0.0, true, 1.0);
	...
	AutoExecConfig(true, "[CSGO-CSS]HNS-Anti-Frag");
}
...
public Action OneHitDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	...
	{	
		...
		if(g_bIsCSS == true)
		{
			if (StrContains(weapon, "knife", false) != -1 && GetClientTeam(attacker) == CS_TEAM_CT && GetClientTeam(victim) == CS_TEAM_T && RoundEnd == false)
			{
				...
				int GetHealth = GetClientHealth(victim) - RoundToFloor(damage);
				...
				}else if(g_bnotify2 == 2)
				{
					if(GetHealth >= 1)
					{
						CPrintToChatAll(" %t","CooldownCTAnnouncedWithHP", attacker, victim, GetHealth);
					}else
					{
		...
		}else if(g_bIsCSGO == true)
		{
			if((damagetype & DMG_SLASH) && GetClientTeam(attacker) == CS_TEAM_CT && GetClientTeam(victim) == CS_TEAM_T && RoundEnd == false)
			{
				int GetHealth;
				if(h_fknife_damage <= 5.0)
				{
					damage = h_fknife_damage + 1.0;
					GetHealth = GetClientHealth(victim) - RoundToFloor(damage) + 1;
				}else if(h_fknife_damage <= 11.0)
				{
					damage = h_fknife_damage + 2.0;
					GetHealth = GetClientHealth(victim) - RoundToFloor(damage) + 2;
				}else if(h_fknife_damage <= 17.0) 
				{
					damage = h_fknife_damage + 3.0;
					GetHealth = GetClientHealth(victim) - RoundToFloor(damage) + 3;
				}else if(h_fknife_damage <= 22.0) 
				{
					damage = h_fknife_damage + 4.0;
					GetHealth = GetClientHealth(victim) - RoundToFloor(damage) + 4;
				}else if(h_fknife_damage <= 27.0) 
				{
					damage = h_fknife_damage + 5.0;
					GetHealth = GetClientHealth(victim) - RoundToFloor(damage) + 5;
				}else if(h_fknife_damage <= 32.0) 
				{
					damage = h_fknife_damage + 6.0;
					GetHealth = GetClientHealth(victim) - RoundToFloor(damage) + 6;
				}else if(h_fknife_damage <= 38.0) 
				{
					damage = h_fknife_damage + 7.0;
					GetHealth = GetClientHealth(victim) - RoundToFloor(damage) + 7;
				}else if(h_fknife_damage <= 44.0) 
				{
					damage = h_fknife_damage + 8.0;
					GetHealth = GetClientHealth(victim) - RoundToFloor(damage) + 8;
				}else if(h_fknife_damage <= 50.0)
				{
					damage = h_fknife_damage + 9.0;
					GetHealth = GetClientHealth(victim) - RoundToFloor(damage) + 9;
				}else if(h_fknife_damage <= 56.0)
				{
					damage = h_fknife_damage + 10.0;
					GetHealth = GetClientHealth(victim) - RoundToFloor(damage) + 10;
				}else if(h_fknife_damage <= 62.0)
				{
					damage = h_fknife_damage + 11.0;
					GetHealth = GetClientHealth(victim) - RoundToFloor(damage) + 11;
				}else if(h_fknife_damage <= 68.0)
				{
					damage = h_fknife_damage + 12.0;
					GetHealth = GetClientHealth(victim) - RoundToFloor(damage) + 12;
				}else if(h_fknife_damage <= 73.0)
				{
					damage = h_fknife_damage + 13.0;
					GetHealth = GetClientHealth(victim) - RoundToFloor(damage) + 13;
				}else if(h_fknife_damage <= 78.0)
				{
					damage = h_fknife_damage + 14.0;
					GetHealth = GetClientHealth(victim) - RoundToFloor(damage) + 14;
				}else if(h_fknife_damage <= 84.0)
				{
					damage = h_fknife_damage + 15.0;
					GetHealth = GetClientHealth(victim) - RoundToFloor(damage) + 15;
				}else if(h_fknife_damage <= 90.0)
				{
					damage = h_fknife_damage + 16.0;
					GetHealth = GetClientHealth(victim) - RoundToFloor(damage) + 16;
				}else if(h_fknife_damage <= 96.0)
				{
					damage = h_fknife_damage + 17.0;
					GetHealth = GetClientHealth(victim) - RoundToFloor(damage) + 17;
					
				}else if(h_fknife_damage <= 102.0)
				{
					damage = h_fknife_damage + 18.0;
					GetHealth = GetClientHealth(victim) - RoundToFloor(damage) + 18;
				}
